...
_rp2040_nets = {
    ...
    # RP2040 无单一 "VDD" 脚 (datasheet): 3V3 域供 IOVDD/USB_VDD/ADC_AVDD/VREG_VIN;
    # 内核 1.1V 由片上 LDO VREG_VOUT 输出, 回灌 DVDD (DVDD_CORE 网)。
    "VCC_3V3":    [("U3","5"),("C9","1"),("U1","IOVDD"),("U1","USB_VDD"),("U1","ADC_AVDD"),("U1","VREG_VIN"),
                   ("C3","1"),("C4","1"),("C5","1"),("C6","1"),("R3","1"),("R1","2"),("R2","2")],
    "DVDD_CORE":  [("U1","VREG_VOUT"),("U1","DVDD")],
    "GND":        [("J1","GND"),("C3","2"),("C4","2"),("C5","2"),("C6","2"),("C7","2"),("C8","2"),("C9","2"),("U1","GND"),("U2","GND"),("SW1","2"),("SW2","2"),("D1","2")],
    # BOOTSEL 按钮接 QSPI 片选 (QSPI_SS), 上电时拉低进 USB 烧录模式 (同树莓派 Pico);
    # 原网表误接到 QSPI_SD3(=FLASH_D3 同一焊盘 51)造成短路, 归正到 QSPI_SS。
    "FLASH_CS":   [("U1","QSPI_CS"),("U2","CS"),("SW1","1")],
    ...
    "RUN":        [("U1","RUN"),("SW2","1")],
    # 电源指示 LED: 3V3 → R3(限流) → D1 阳极 → GND。原网表把 D1 阳极同时挂在 3V3 与
    # LED_PWR 造成焊盘 1 重复, 归正为串联拓扑。
    "LED_PWR":    [("R3","2"),("D1","1")],
}
...
_safety_protection_nets = {
    ...
    "5V_IN":     [("F2","1")],
    "5V":        [("F2","2"),("D2","K"),("D4","K")],
    "3V3":       [("D3","K"),("U4","VDD"),("R1","1"),("R2","2"),("C1","1"),("C2","1"),("C5","1"),("C6","1"),("U1","VBUS")],
    "GND":       [("D1","A"),("D2","A"),("D3","A"),
    ...
    "CAN_IO1":   [("U3","IO1")],
    "CAN_IO2":   [("U3","IO2")],
    # TPS3823-33 仅有 VDD/GND/~RESET/WDI/~MR 五脚 (datasheet); 原网表虚构的 WDO_N/CT 脚
    # 不存在(那是 TPS3813 等带可调超时型号的脚)→删除。R2(0Ω)归正为 ~MR 上拉(接 3V3,
    # 不用手动复位); R1(10k)作 WDI 上拉。C2 归为 3V3 去耦电容。
    "WDI":       [("U4","WDI"),("R1","2")],
    "MR":        [("U4","MR"),("R2","1")],
    "NRST":      [("U4","RESET_N")],
}
...
_industrial_power_components = [
    ...
    Comp("R3",  "100k",         "passive", (20.0, 25.0)),
...
_industrial_power_nets = {
    "12V":   [("J1","1"),("C1","1"),("C2","1"),("U1","VIN"),("R3","1")],
...
_aerial_components = [
    ...
    Comp("U3",  "INA226AIDGST",   "Package_SO",              "TSSOP-10_3x3mm_P0.5mm",               (14, 15), "mcu",       "电流/电压监控INA226(I2C 16bit)"),
...
_aerial_nets = {
    # ── 电源层 (高边电流采样: 电池→保险丝→分流电阻 R1→被监控负载轨) ────────
    # INA226 跨 R1(1mΩ) 测电流: Vin+→分流上游(VBAT_FUSED)、Vin-/Vbus→下游(VBUS_SENSE);
    # BEC(U1)+分压(R12)挂下游, 故其电流被计入。原网表把 R1/U3 同一焊盘塞进 2 个网
    # (短路), 且按虚构数字脚引用 INA226(实为 TSSOP-10): 据 KiCad 官方符号归正为功能名。
    "VBAT":      [("J1","1"),  ("F1","1"),  ("C1","1"),  ("C2","1"),  ("TVS1","1")],
    "VBAT_FUSED":[("F1","2"),  ("R1","1"),  ("U3","Vin+")],
    "VBUS_SENSE":[("R1","2"),  ("U3","Vin-"), ("U3","Vbus"), ("U1","3"),  ("C3","1"),  ("R12","1")],
    "VCC_5V":    [("U1","2"),  ("L1","2"),  ("C4","1"),  ("C5","1"),  ("C6","1"),  ("U2","1"),  ("J3","2"),  ("J4","2"),  ("J5","2"),  ("J6","2"),  ("J7","2"),  ("J8","2"),  ("J9","2"),  ("J10","2"), ("J11","2"), ("J12","1")],
    # U6(ICM-42688-P) pad1=AP_SDO(MISO) 非电源, U7(ICM-20602) VDD=16/VDDIO=1 双供,
    # 故 3V3 按功能名接各芯片真实电源脚; LED 限流 R10 上端接 3V3 (原误把 D2 阳极直挂)。
    "VCC_3V3":   [("U2","2"),  ("C7","1"),  ("C8","1"),  ("C9","1"),  ("C10","1"), ("U4","1"),  ("U3","VS"), ("U5","1"),  ("U6","VDD"), ("U6","VDDIO"), ("U7","VDDIO"), ("U7","VDD"), ("U8","VDD"), ("C14","1"), ("C15","1"), ("C16","1"), ("C17","1"), ("C18","1"), ("C19","1"), ("C20","1"), ("C21","1"), ("C22","1"), ("C23","1"), ("C24","1"), ("C25","1"), ("R2","1"),  ("R6","1"),  ("R7","1"),  ("R10","1"), ("J12","2"), ("C11","1"), ("C26","1"), ("R4","1")],
    # 修正: TVS1(SMAJ28A)为 2 焊盘 D_SMA, 阴极(1)→VBAT、阳极(2)→GND。INA226 的 GND/A0/A1
    # 接地(地址 0x40); IMU/气压计的 GND 按真实焊盘 (U6=6/U7=15/U8=3), MS5611 PS→GND 选 SPI;
    # I2C 上拉 R6/R7 下端属信号网非地; LED 阴极 D2/D3 入地。J2(USB-C)用真实 A1=GND。
    "GND":       [("J1","2"),  ("C1","2"),  ("C2","2"),  ("TVS1","2"),("U1","1"),  ("D1","2"),  ("C3","2"),  ("C4","2"),  ("C5","2"),  ("C6","2"),  ("U2","3"),  ("C7","2"),  ("C8","2"),  ("C9","2"),  ("C10","2"), ("U3","GND"), ("U3","A0"), ("U3","A1"), ("C11","2"), ("U4","2"),  ("C12","2"), ("C13","2"), ("C14","2"), ("C15","2"), ("C16","2"), ("C17","2"), ("SW1","2"),  ("U5","2"),  ("C18","2"), ("U6","GND"), ("C19","2"), ("C20","2"), ("C21","2"), ("U7","GND"), ("C22","2"), ("C23","2"), ("U8","GND"), ("U8","PS"), ("C24","2"), ("C25","2"), ("D2","2"),  ("D3","2"),  ("J3","3"),  ("J4","3"),  ("J5","3"),  ("J6","3"),  ("J2","A1"),  ("C26","2"), ("R13","2"), ("C27","2"), ("J12","4")],
    # ── MCU时钟 ───────────────────────────────────────────────
    ...
    # ── SPI1: 主IMU ICM-42688-P (4线SPI, 据 TDK DS-000347 功能名解析) ─────
    "SPI1_SCK":  [("U4","10"), ("R5","1")],
    "SPI1_SCK_F":[("R5","2"),  ("U6","AP_SCLK")],
    "SPI1_MOSI": [("U4","11"), ("U6","AP_SDI")],
    "SPI1_MISO": [("U4","12"), ("U6","AP_SDO")],
    "SPI1_CS1":  [("U4","13"), ("U6","AP_CS")],
    "IMU1_INT":  [("U6","INT1"), ("U4","14")],
    # ── SPI2: 备份IMU ICM-20602 (SCLK=SCL/2 MOSI=SDA/3 MISO=SAO/4 CS=5) ──
    "SPI2_SCK":  [("U4","20"), ("U7","SCL")],
    "SPI2_MOSI": [("U4","21"), ("U7","SDA")],
    "SPI2_MISO": [("U4","22"), ("U7","SAO")],
    "SPI2_CS2":  [("U4","23"), ("U7","CS")],
    # ── SPI3: 气压计 MS5611 (SCLK=8 SDI=7 SDO=6 CSB=5) ────────────────────
    "SPI3_SCK":  [("U4","30"), ("U8","SCLK")],
    "SPI3_MOSI": [("U4","31"), ("U8","SDI")],
    "SPI3_MISO": [("U4","32"), ("U8","SDO")],
    "SPI3_CS3":  [("U4","33"), ("U8","CSB")],
    # ── I2C1: INA226电流监控 (共用总线) ─────────────────────
    "I2C1_SDA":  [("U4","40"), ("U3","SDA"), ("R6","2")],
    "I2C1_SCL":  [("U4","41"), ("U3","SCL"), ("R7","2")],
    "INA226_ALT":[("U3","Alert"), ("U4","42")],
    # ── 电机PWM/DSHOT输出 ─────────────────────────────────────
